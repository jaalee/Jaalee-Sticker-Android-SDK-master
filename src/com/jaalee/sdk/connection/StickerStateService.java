 ...
 */
public class StickerStateService
  implements BluetoothService
{
	...
	public void processGattServices(List<BluetoothGattService> services)
	{
		for (BluetoothGattService service : services)
			if (JaaleeUuid.STICKER_STATE_SERVICE.equals(service.getUuid())) {
				if (service.getCharacteristic(JaaleeUuid.STICKER_STATE_CHAR) != null)
				{
					this.characteristics.put(JaaleeUuid.STICKER_STATE_CHAR, service.getCharacteristic(JaaleeUuid.STICKER_STATE_CHAR));	
				}	
			}
	}
	...
	public Integer getBeaconState() {
		...
		{
				return this.characteristics.containsKey(JaaleeUuid.STICKER_STATE_CHAR) ? Integer.valueOf(getUnsignedByte(((BluetoothGattCharacteristic)this.characteristics.get(JaaleeUuid.STICKER_STATE_CHAR)).getValue())) : null;	
		}
		return 1;		
	}
