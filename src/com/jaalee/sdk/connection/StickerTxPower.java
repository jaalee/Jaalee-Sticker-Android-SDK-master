  ...
  implements BluetoothService
{
	...
	public void processGattServices(List<BluetoothGattService> services)
	{
		for (BluetoothGattService service : services)
			if (JaaleeUuid.STICKER_TX_POWER_SERVICE.equals(service.getUuid())) {
				if (service.getCharacteristic(JaaleeUuid.STICKER_TX_POWER_CHAR) != null)
				{
					this.characteristics.put(JaaleeUuid.STICKER_TX_POWER_CHAR, service.getCharacteristic(JaaleeUuid.STICKER_TX_POWER_CHAR));	
				}
			}
	}
	...
	public Integer getBeaconTxPower() {
		int temp = this.characteristics.containsKey(JaaleeUuid.STICKER_TX_POWER_CHAR) ? Integer.valueOf(getUnsignedByte(((BluetoothGattCharacteristic)this.characteristics.get(JaaleeUuid.STICKER_TX_POWER_CHAR)).getValue())) : null;
		int txPower;
		if (mCurrentIsJaaleeNewBeacon)
		{
			switch(temp)
